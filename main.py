...
class ImagePair:
    def __init__(self, img1_path, img2_path, M_anchors = np.eye(3)):
        ...
        self.initialize_from_homography(np.linalg.inv(self.M_original) @ M_anchors @ self.M_original)
        self.save_state()
    ...
    def initialize_from_homography(self, H):
        H_ = self.M_original @ H @ np.linalg.inv(self.M_original)
        translation, rotation, scale, H2 = decompose_homography(H_, self.img2.shape, self.scale_ratio)
...
class ButtonPanel:
    ...
    def create_widgets(self):
        ...
        self.grid_button = tk.Button(self.frame, text="Show Grid: OFF", command=self.app.toggle_grid)
